...
git config --global alias.bundle-fetch '!f() { ROOT="$(git rev-parse --show-toplevel)" || { echo "git repository error" 1>&2 ; exit 1 ; } ; BUNDIR="${ROOT}/.git/bundle" ; if [ ! -d "${BUNDIR}" ] ; then mkdir -p "${BUNDIR}" ; chmod -R go-rwx "${BUNDIR}" ; fi ; eval "FETCH="" ; $(if [ -f "${BUNDIR}/git.bundle.env" ] ; then grep -E "^\s*(FETCH)=" "${BUNDIR}/git.bundle.env" ; fi )" ; [ -n "$1" ] && { if [ -z "${FETCH}" ] ; then git bundle-config FETCH "$1" ; fi ; FETCH="$1" ; } ; [ -f "${FETCH}" ] && FETCH="file://$(readlink -f "${FETCH}")" ; eval "DIGEST=sha256 ; KEY="" ; $(if [ -f "${BUNDIR}/git.bundle.env" ] ; then grep -E "^\s*(DIGEST|KEY)=" "${BUNDIR}/git.bundle.env" ; fi )" ; OPTIONS="-md "${DIGEST}"" ; [ -n "${KEY}" ] && OPTIONS=" -k "${KEY}"" ; if [ -n "${FETCH}" ] ; then echo "FETCH=${FETCH} ; OPTIONS=${OPTIONS}" ; curl --silent -L "${FETCH}" | openssl enc -d -aes-256-cbc $OPTIONS > "${BUNDIR}/git.bundle" 2>/dev/null ; [ "$?" -ne 0 ] && echo "ERR!: decryption error" 1>&2 || git bundle unbundle "${BUNDIR}/git.bundle" ; else echo "ERR!: missing FETCH/PULL source" 1>&2 ; exit 1 ; fi ; } ; f'
